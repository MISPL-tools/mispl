...
const VERSION = "v2.70.0 - The IntelliSense & Global Refactoring Update";
...
// 🛡️ MASKER FUNCTIE NAAR BOVEN VERPLAATST ZODAT HIJ ALTIJD BEKEND IS
        ...
        for (let vKey of varKeys) {
            ...
            if (vInfo && vInfo.dataType && vInfo.dataType !== "UNKNOWN") {
                ...
                if (regexTest.test(work)) {
                    ...
                    if (isCheckingNull) {
                        // Stilzwijgend toestaan! (Oplossing v2.60.6)
                        ...
                        // Primitieve basistypes roepen we GEEN error of warning op (Oplossing v2.60.5)
    ...
    analyzeLoops(nodes, context) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (loopCondition && blockEnd > -1) {
                ...
                // PAS 1: Verzamel eerst ALLE variabelen die ergens in deze loop gewijzigd worden
                ...
                // PAS 2: Analyseer zware functies
                for (let m = i + 1; m < blockEnd; m++) {
                    const subNode = nodes[m];
                    let exprToCheck = subNode.value || subNode.condition || subNode.expression || subNode.text;
                    if (exprToCheck) {
                        ...
                        while ((match = heavyFunctions.exec(safeExpr)) !== null) {
                            ...
                            if (!isDependent) context.addInfo(subNode.line, `⚡ Prestatie-tip: '${match[1]}' staat in een loop maar argumenten wijzigen niet. Bereken dit 1x voor de loop en gebruik een variabele!`);
                        }
                    }
                }

                // PAS 3: Oneindige loop check
