...
class MisplCodeActionProvider {
	provideCodeActions(document, range, context, token) {
		...
		if (context.diagnostics.length > 0) {
			...
			// Bied de 'Ignore' optie alleen aan als hij er niet al toevallig staat
			if (!lineText.includes("/*Ign@re*/")) {
				// Maak de Quick Fix aan
				const ignoreFix = new vscode.CodeAction(
					t('ACTION_IGNORE_WARNING'), // Vertaalde tekst: "Negeer meldingen op deze regel (/*Ign@re*/)"
					vscode.CodeActionKind.QuickFix
				);

				// Vertel VS Code wat deze fix precies moet doen (tekst toevoegen)
				ignoreFix.edit = new vscode.WorkspaceEdit();

				// We plakken de tag exact aan het einde van de regel
				ignoreFix.edit.insert(document.uri, line.range.end, ' /*Ign@re*/');

				// Zorg dat deze optie NIET bovenaan staat (bijv. "Ongebruikte variabelen wissen" is vaak belangrijker)
				ignoreFix.isPreferred = false;

				// Voeg hem toe aan de lijst met acties die VS Code laat zien
				actions.push(ignoreFix); // (Of return [ignoreFix, ...], afhankelijk van hoe je array heet)
			}
		}
